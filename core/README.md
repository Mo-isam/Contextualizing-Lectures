...
* **`models.py`**: The strict source of truth for data structures (e.g., `AlignedNote`, `LectureSession`). Features like `session_description` are strictly typed here before touching the UI.
...
* **`audio_processor.py`**: Manages FFmpeg media extractions and Whisper/Gemini routing. It directly intercepts and patches the global `tqdm` module to stream live, frame-by-frame progress to the UI without coupling to Streamlit.
